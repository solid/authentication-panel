...
* AC: Rhetorical Question: when M was describing implementation specific code.. as more implemetations come along.. I would forsee a lot more impl specific code. Is there a structure that we can develop that will allow us to avoid so much impl-specific code? .. For example: within a single test suite.
...
* AC: this gets to Pete's question. what is the mechanism by which CLI-based test suites interact with an RS. one of the challenges with OpenID Connect is that there are many permutations (e.g., client credentials, refresh tokens). b/c we are building on top of OpenID connect.. we are only mandating authorization code flow. some of the identity systems do these other mechanisms.. providing facilities to refresh .. test suite can make use this .. but not sure if it needs to be standardised.
...
* AC: makes sense.. any metadata to identify what we are testing.. (as well as how we are testing: manual or automated)
...
* AC: should we try to wrap up this discussion and continue next week.. or press on? we also have open PRs that we need to go through. Noting eP's +1 to prioritze testing due to participation
...
* AC: there's been some work in the past: basically create a mock IdP.. that might be helpful in these RS-OP interactions.. it generates a token .. with limited access for that webid, you can then begin testing that RS, one thing that may make things easier: build out or reuse.. existing infra. has mock idp.. that generates valid access tokens for a well known webid, if the RS under test gives limited permissions to that well known webid, then you can begin to test RS. There may be prior art for this, but we would need to verify conformance with the current state of Solid-OIDC.
