...
* Aaron: If you have issuer example.com and your WebID is at user.example.com so its on subdomain of IdP. Should there be implicit trust between the WebId and the issuer.
* Barath: What implicit trust would be? ...
...
* Barath: You could turn in in positive and negative. On one way it can be seen as coersion. With explicit requirement you could also go with another IdP.
...
* Barath: It raises this 'bring your own domain' portability question. This ends up relating as 'flip side' to the question. You should be able to host anywhere. It actually changes the implicit trust. If you have IdP on example.com, I try to assess what trust you have to exapmle.com . If there is virtual hosting it would affect the implicit trust. Whoever you use as IdP you make a statement of trusting them.
* Aaron: Than it becomes almost an implementation constraint than specification issue. Once you get into handing domains for people, all kind of issues can appear.
...
* Pavlik: This brings to my mind browser-list of top level domain, since regex doesn't exit to tell difference between subdomain and domain with multi part tld. https://publicsuffix.org/
...
* Barath: The fewer variation the better. Even if we call out implicit trust it's still better to keep fewer variations.
* ...: Is the proposal that existing flow is changed?
* Aaron: This would affect RS, can they use implicit trust or the have to deference WebId. Also NSS doesn't produce that triple in WebID document.
* Pavlik: Is there a reason or NSS was just assuming that implicit trust and didn't bother to add explicit statement.
...
* Aaron: I just want to make sure to argue this point. Wherever there is breaking change some people will not like that.A
...
* Pavlik: what is the purpose of verifying client identity in Solid-OIDC?  I see it used for authZ, where else we see it used?

* Aaron: Let's imagine that you have a storage with different types of information. Photos, medical data, financial data etc. As owner you have access to all that data. If you use malicious or untrusted client you want to restrict what this client can do. That said, Solid-OIDC defines it only before we don't have better way of defining it. WAC was using origin header which has it's own problems. GNAP will come along eventually, what happens then if we tie everything to OIDC. I don't think those mechanisms need to be exclusive. 
