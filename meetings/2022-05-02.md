...
* AC: There are so many permutations since there are so many contexts where clients try to authenticate. Give all of them, we can't always say that client MUST do it this way. We are also trying to move ecosystem along in a step by step fashion. If we don't allow other mechanisms, then we get into compatibility issues.
